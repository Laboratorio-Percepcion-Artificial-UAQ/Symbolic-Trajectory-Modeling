...
%% === Load Video ===
vid = VideoReader(fullfile(video_path,'video_unido.avi'));
...
subplot(2,2,2); hMotion = imshow(Im_prev_gray); title('Motion Mask');
subplot(2,2,3); Im_path = zeros(size(Im_prev),'uint8');
                hPaths  = imshow(Im_path);   title('Tracking');
subplot(2,2,4); hStates = imshow(state_mask); title('Accumulated States');
...
while hasFrame(vid)
                    ...
                    if (frame_idx - last_update(key)) > dead_frame_limit
                        ...
                        if size(Tracks_temp(key),1) > 3
                            ...
                            if mod(Tracks_final.Count,40)==0
                                ...
                                Track_symbols = convertirTrayectoriasAsimbolos(Tracks_final, vec);
                                ...
                                for t_sym = 1:length(symbol_names)
                                    ...
                                    id_num = str2double(name_trk(6:end));
                                    ...
                                    if length(unique_sym) > 1
                                        ...
                                        Rul = reglas_decodificadas(R);

                                        Rules_aux = [Rules_aux, Rul];
                                        Rules_aux = eliminar_reglas_repetidas(Rules_aux);
                                    end
    ...
    'vec','ProductionRules','Rules_aux','Track_symbols','Tracks_final','state_mask');
        ...
        for kk = 1:length(temp_keys)
            ...
            if size(pts,1) > 1
                for t = 1:size(pts,1)-1
                    p1 = pts(t,1:2);
                    p2 = pts(t+1,1:2);
                    Im_path = insertShape(Im_path,'Line',...
                        [p1(2),p1(1), p2(2),p2(1)],...
        ...
        %% === UPDATE FIGURES ===
        set(hVideo,'CData',Im);
        set(hMotion,'CData',uint8(motion_mask)*255);
        set(hPaths,'CData',Im_path);
